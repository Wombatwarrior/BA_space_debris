...
struct FileInput::TxtLineContent
FileInput::tokenizeLine(const std::string &line) {
  TxtLineContent l;
...
void FileInput::setDebrisValues(Debris::Debris &d, const std::string &line) {
  ...
  std::string velocity_str =
      line.substr(position_split_pos + 1, velocity_split_pos);
...
Debris::DebrisContainer &FileInput::getDebris() { return *debris; }

void FileInput::setDebris(Debris::DebrisContainer &debris) {
  FileInput::debris = &debris;
}

std::string &FileInput::getInputFileName() { return input_file_name; }

void FileInput::setInputFileName(std::string &inputFileName) {
  input_file_name = inputFileName;
}

FileInput::Type FileInput::getInputFileType() { return input_file_type; }

void FileInput::setInputFileType(FileInput::Type inputFileType) {
  input_file_type = inputFileType;
}

double FileInput::getDeltaT() { return delta_t; }

void FileInput::setDeltaT(double deltaT) { delta_t = deltaT; }

double FileInput::getWriteDeltaT() { return write_delta_t; }

void FileInput::setWriteDeltaT(double writeDeltaT) {
  write_delta_t = writeDeltaT;
}

double FileInput::getStartT() { return start_t; }

void FileInput::setStartT(double startT) { start_t = startT; }

double FileInput::getEndT() { return end_t; }

void FileInput::setEndT(double endT) { end_t = endT; }

std::array<bool, 8> &FileInput::getAccConfig() { return acc_config; }
