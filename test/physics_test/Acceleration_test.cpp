...
/**
 * Tests if the acceleration calculated using Acceleration::J2Component::apply()
 * is the same as using another implementation
 */
...
/**
 * Tests if the acceleration calculated using Acceleration::J2Component::apply()
 * is the same as some hand calculated values
 */
...
TEST_F(LunComponentTests, CalculationEquivalenceTest) {
  ...
  for (int j = 0; j < 100; ++j) {
    std::array<double, 6> moon_params =
        Acceleration::LunComponent::setUp(t * j);
...
/**
 * Tests if the acceleration calculated using
 * Acceleration::LunComponent::apply() is the same as some and calculated values
 */
...
TEST_F(LunComponentTests, CompareOptimizedSetupFunctionResult) {
  ...
  for (int i = 0; i < 10; ++i) {
    const std::array<double, 6> params_1 =
        Acceleration::LunComponent::setUp(t * i);
...
TEST_F(LunComponentTests, CompareTrigonometricTerms) {
  ...
  // no calls
  double r1 = -152 * ((c_lm * c_l1m - s_lm * s_l1m) * c_2dm +
                      (s_lm * c_l1m + c_lm * s_l1m) * s_2dm);
  ...
  ASSERT_NEAR((c_lm * c_l1m - s_lm * s_l1m) * c_2dm +
                  (s_lm * c_l1m + c_lm * s_l1m) * s_2dm,
  ...
  lambda1 -= (206.0 / 3600) * ((s_lm * c_l1m + c_lm * s_l1m) * c_2dm -
                               (c_lm * c_l1m - s_lm * s_l1m) * s_2dm);
  ...
  // 14 calls
  double lambda2 =
      -(55.0 / 3600) * std::sin((2 * f_m - 2 * d_m) * M_PIf64 / 180);
  ...
  ASSERT_NEAR((s_lm * c_l1m + c_lm * s_l1m) * c_2dm -
                  (c_lm * c_l1m - s_lm * s_l1m) * s_2dm,
  ...
  // 1 call
  double beta1 = (11.0 / 3600) * ((s_fm * c_l1m - c_fm * s_l1m) * c_2dm -
                                  (c_fm * c_l1m + s_fm * s_l1m) * s_2dm);
  beta1 += (21.0 / 3600) * (s_fm * c_lm - c_fm * s_lm);
  beta1 -= (23.0 / 3600) * ((s_l1m * c_fm + c_l1m * s_fm) * c_2dm -
                            (c_l1m * c_fm - s_l1m * s_fm) * s_2dm);
  beta1 -= (25.0 / 3600) * (s_fm * c_2lm - c_fm * s_2lm);
  beta1 -= (31.0 / 3600) * ((s_fm * c_lm - c_fm * s_lm) * c_2dm -
                            (c_fm * c_lm + s_fm * s_lm) * s_2dm);
  beta1 += (44.0 / 3600) * ((s_lm * c_fm + c_lm * s_fm) * c_2dm -
                            (c_lm * c_fm - s_lm * s_fm) * s_2dm);
  beta1 -= (526.0 / 3600) * (s_fm * c_2dm - c_fm * s_2dm);
  beta1 += (18520.0 / 3600) *
           std::sin((f_m + lambda1 - l_0 + (412.0 / 3600) * s_2fm +
                     (541.0 / 3600) * s_l1m) *
                    M_PIf64 / 180);

  // 10 calls
  double beta2 =
      (11.0 / 3600) * std::sin((-l1_m + f_m - 2 * d_m) * M_PIf64 / 180);
  ...
  beta2 += (18520.0 / 3600) *
           std::sin((f_m + lambda2 - l_0 +
                     (412.0 / 3600) * std::sin((2 * f_m) * M_PIf64 / 180) +
                     (541.0 / 3600) * std::sin((l1_m)*M_PIf64 / 180)) *
                    M_PIf64 / 180);

  ASSERT_NEAR(std::sin((f_m + lambda1 - l_0 + (412.0 / 3600) * s_2fm +
                        (541.0 / 3600) * s_l1m) *
                       M_PIf64 / 180),
              std::sin((f_m + lambda2 - l_0 +
                        (412.0 / 3600) * std::sin((2 * f_m) * M_PIf64 / 180) +
...
/**
 * Tests if the acceleration calculated using
 * Acceleration::SolComponent::apply() is the same as some and calculated values
 */
...
TEST_F(SolComponentTests, CompareSetupFunction) {
  ...
  for (int i = 0; i < 10; ++i) {
    const std::array<double, 6> params_1 =
        Acceleration::SolComponent::setUp(t * i);
...
/**
 * Compares the result of the Acceleration::SolComponent::apply() function using
 * the calcSolParams() function instead of Acceleration::SolComponent::setUp()
 */
