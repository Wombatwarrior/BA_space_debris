...
protected:
  ...
  void calcJ2(Debris::Debris &d, std::array<double, 3> &acc_j2) {
    ...
    double n3 = 15 * (z * z);
    double d3 = (x * x + y * y + z * z) * (x * x + y * y + z * z) *
                (x * x + y * y + z * z);
...
protected:
  ...
  const std::array<double, 6> calcMoonParams(double t) {
    ...
    double r_m = -152 * ((c_lm * c_l1m - s_lm * s_l1m) * c_2dm +
    ...
    lambda_m -= (206.0 / 3600) * ((s_lm * c_l1m + c_lm * s_l1m) * c_2dm -
    ...
    double beta_m = (11.0 / 3600) * ((s_fm * c_l1m - c_fm * s_l1m) * c_2dm -
                                     (c_fm * c_l1m + s_fm * s_l1m) * s_2dm);
    beta_m += (21.0 / 3600) * (s_fm * c_lm - c_fm * s_lm);
    beta_m -= (23.0 / 3600) * ((s_l1m * c_fm + c_l1m * s_fm) * c_2dm -
                               (c_l1m * c_fm - s_l1m * s_fm) * s_2dm);
    beta_m -= (25.0 / 3600) * (s_fm * c_2lm - c_fm * s_2lm);
    beta_m -= (31.0 / 3600) * ((s_fm * c_lm - c_fm * s_lm) * c_2dm -
                               (c_fm * c_lm + s_fm * s_lm) * s_2dm);
    beta_m += (44.0 / 3600) * ((s_lm * c_fm + c_lm * s_fm) * c_2dm -
                               (c_lm * c_fm - s_lm * s_fm) * s_2dm);
    beta_m -= (526.0 / 3600) * (s_fm * c_2dm - c_fm * s_2dm);
    beta_m += (18520.0 / 3600) *
              std::sin((f_m + lambda_m - l_0 + (412.0 / 3600) * s_2fm +
                        (541.0 / 3600) * s_l1m) *
                       M_PIf64 / 180);
    ...
    double d2 = moon_params[0] * moon_params[0] +
                moon_params[1] * moon_params[1] +
                moon_params[2] * moon_params[2];
  ...
  void calcLun(Debris::Debris &d, double t, std::array<double, 3> &acc_lun) {
    ...
    lambda_m -=
        (206.0 / 3600) * std::sin((l_m + l1_m - 2 * d_m) * M_PIf64 / 180);
...
protected:
  ...
  void calcSol(Debris::Debris &d, double t, std::array<double, 3> &acc_sol) {
    ...
    double r = 149.619 - 2.499 * std::cos(l * M_PIf64 / 180) -
               0.021 * std::cos(2 * l * M_PIf64 / 180);
    double lambda = ap + lan + l +
                    (6892.0 / 3600) * std::sin(l * M_PIf64 / 180) +
                    (72.0 / 3600) * std::sin(2 * l * M_PIf64 / 180);
    double xs = r * std::cos(lambda * M_PIf64 / 180);
    double ys =
        r * std::sin(lambda * M_PIf64 / 180) * std::cos(e * M_PIf64 / 180);
    double zs =
        r * std::sin(lambda * M_PIf64 / 180) * std::sin(e * M_PIf64 / 180);
    xs *= 1e+6;
    ys *= 1e+6;
    zs *= 1e+6;
  ...
  const std::array<double, 6> calcSolParams(double t) {
    ...
    sol_params[2] *= 1e+6;
    double d2 =
        std::pow(sol_params[0] * sol_params[0] + sol_params[1] * sol_params[1] +
                     sol_params[2] * sol_params[2],
                 1.5);
